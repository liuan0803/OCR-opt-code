...
# 读取txt文件
train_txt = open(
    '/data/liuan/jupyter/root/project/keras-retinanet-master/bbox_fz_zc_006000new/dataset/txtsave/train.txt')
...
for linetr in train_content:
    ...
    ratio = np.array([imgresize.shape[0] / img.shape[0], imgresize.shape[1] / img.shape[1]],
                     np.float32)  # height缩放比 ,width 缩放比，
...
val_xml_resize = []
for linetr in train_content:
    xml_path = '/data/liuan/jupyter/root/project/keras-retinanet-master/bbox_fz_zc_006000new/dataset/xml_zc_fz_6000_clean/' + linetr.rstrip(
        '\n') + '.xml'
    print(xml_path)
...
for lineva in val_content:
    xml_path = '/data/liuan/jupyter/root/project/keras-retinanet-master/bbox_fz_zc_006000new/dataset/xml_zc_fz_6000_clean/' + lineva.rstrip(
        '\n') + '.xml'
    print(xml_path)
    xml_DomTree = xml.dom.minidom.parse(xml_path)
    xml_annotation = xml_DomTree.documentElement
...
pb_file_path = '/data/liuan/jupyter/root/project/keras-retinanet-master/bbox_fz_zc_006000new/bbox_pb_model1/ocr_bbox_batch16_epoch'
...
writer = tf.summary.FileWriter(
    "/data/liuan/jupyter/root/project/keras-retinanet-master/bbox_fz_zc_006000new/record_graph", sess.graph_def)
...
for epoch in range(n_epoch):
    ...
    with tf.gfile.FastGFile(
            pb_file_path + '_' + str(epoch) + '_' + 'train_acc' + str(train_acc / n_batch) + 'val_acc' + str(
                    val_acc / n_batch) + '.pb', mode='wb') as f:
